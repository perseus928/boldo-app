...
export default class Pending extends Component {
    ...
    updateSearch = (search) => {
        let filteredData = this.state.pendings.filter(x => String(x.user.fname.toLowerCase()).includes(search.toLowerCase()) ||
            String(x.user.lname.toLowerCase()).includes(search.toLowerCase()));
        this.setState({
    ...
    getPendings() {
        if (store.getState().auth?.login?.success) {
            ...
            this.setState({
                ...
                apiActions.getConnections(model)
                    .then(response => {
                        this.setState({
                            pendings: response.users,
                            filteredPending: response.users
                        })
                    })
                    .catch(err => {
                        console.log('err');
    ...
    onApply = (item, index) => {
        ...
        this.setState({
            ...
            apiActions.applyPending(model)
                .then(response => {
                    let filteredPending = this.state.filteredPending;
                    filteredPending.splice(index, 1);
                    this.setState({ filteredPending: filteredPending })
                })
                .catch(err => {
                })
                .finally(
                        ...
                        renderItem={({ item, index }) => {
                            return (
                                <View style={styles.chef} >
                                    <Image style={styles.chefAvatar} source={{ uri: item.user.photo }}></Image>
                                    <Text title3 blackColor styles={{ marginTop: 10 }}> {item.user.fname} </Text>
                                    <Text style={{ flex: 1 }} caption1 blackColor> {item.user.typeOfProfessional.length > 0 ? item.user.typeOfProfessional.join(" | ") : "Customer"} </Text>
                                    <Button style={{
                                        height: 35, width: '100%',
                                        marginTop: 10, borderRadius: 8,
                                        backgroundColor: EStyleSheet.value('$redColor')
                                    }}
                                        styleText={{ color: EStyleSheet.value('$whiteColor') }}
                                        onPress={this.onRemove.bind(this, item, index)}
                                    >
                                        {Utils.translate("Account.cancel")}
                                    </Button>
                                    <Button style={{
                                        height: 35, width: '100%',
                                        marginTop: 10, borderRadius: 8,
                                        ...
                                        styleText={{ color: EStyleSheet.value('$whiteColor') }}
                                        onPress={this.onApply.bind(this, item, index)}
                                    >
                                        {Utils.translate("Account.apply")}
                                    </Button>
                                </View>
                            );
                        }}
